...
def read_write_yaml(filename: str, mode: str, data_yaml: dict) -> dict:
...
def merge_configs(all_configs, rl_path=None):

	configs: List[Dict] = []
	for config in all_configs:
		configs.append(read_write_yaml(rl_path + config, 'r', None))

	merged_configs: Dict = {}
	for config in configs:
		merged_configs.update(config)
	
	merged_configs = assert_check_edit_configs(merged_configs)
